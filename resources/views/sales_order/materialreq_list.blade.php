...
@foreach($result as $q)
<?php $no; ?>
<tr style="font-size: 12px !important;" id="row_detail" data-id="{{$q->id}}" data-number="{{$q->number}}"
    data-idsales="{{$q->id_sales_order}}" data-name="{{$no}}">
    ...
    <td>
        ...
        <center>
            <a href="/salesorders/materialrequirements/delete/{{ $q->id }}/{{$id_sales_order}}"
            ...
            <a href="/salesorders/materialrequirements/edit/{{ $q->id }}/{{$id_sales_order}}"
